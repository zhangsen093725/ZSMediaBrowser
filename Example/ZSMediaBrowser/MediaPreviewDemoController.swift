...
class MediaPreviewDemoController: UIViewController, ZSMediaBrowserDelegate, ZSMediaBrowserLoadDelegate {
    ...
    lazy var previewServe: ZSMediaBrowserCustome = {
        
        let previewServe = ZSMediaBrowserCustome()
        previewServe.loadDelegate = self
        return previewServe
    }()
    
    func getModels() -> [ZSMediaBrowserModel] {
        
        var models: [ZSMediaBrowserModel] = []
        ...
        for file in imageFiles {
            let model = ZSImageBrowserModel()
            model.originImage = file
            models.append(model)
        }
        
        for file in audioFile {
            let model = ZSAudioBrowserModel()
            model.URLString = file
            models.append(model)
        }
        
        for file in videoFile {
            let model = ZSVideoBrowserModel()
            model.URLString = file
    ...
    @objc func openMediaPreview() {
        previewServe.mediaBrowserView?.beginPreview(from: imageView, to: 0)
    }
